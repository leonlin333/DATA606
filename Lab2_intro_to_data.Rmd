...
The main difference between these three histograms is the spread of the data. With less bins the majority of the data can go towards one bin making the data look more skewed than it actually is. The bins tells you how to spread the data and by how many groups. Less bins means less groups and thus the size of each bin. You can see the difference in count with 15 bins as opposed to 150 bins. You see that the max count changes from 30000 to around 20000. The amount of bins can tell you the distrubition of data if the bins are correct. Changing the bins however can give the audience the wrong idea in terms of the distribution.
...
ggplot(data=nycflights, aes(x=arr_delay))+
  ...
  xlim(-50, 200)+
  geom_vline(
    xintercept = mean(nycflights$arr_delay),
    color = "red",
    linewidth = 1)
```
Based on the the mean you can see that the distribution of the graph is right skewed. A majority of the data occurred befored the mean(red line). 
...
```
Both DL and UA has the most variable arrival delays with the highest IQR. 
...
**Insert your answer here**
The pros of means is that its a good representation of typcial departure delays and can usually work. The cons is that its sensitve to outliers that may trick you into thinking that the month has has mean departure when it really its low with with just a few days of outliers. The pros of medians is that it typically ignore larger delay outliers that would skew the data. The cons of median is that it might not potentially reflect how bad these delays actually are. 
...
**Insert your answer here**
If I were to select an airport simply based on departure time percentage then I would pick LGA. LGA has the lowest percentage of delays compared to on time. They all pretty much have the same count for on time but the other two airports has much more delays to on time. LGA has the best ratio for on time to delayed. 
...
```{r}
ggplot(data= new_data, aes(x=distance, y=avg_speed))+
  geom_point()+
  geom_smooth(method="lm")
```
Based on the plot you see a positive relationship between distance and average speed. As the trip distance is longer you'll typically see a higher average speed.
