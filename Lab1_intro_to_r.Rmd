...
```{r r-interface-2020, echo=FALSE, results="asis", out.width='100%'}
...
ggplot(data=arbuthnot, aes(x=year, y=girls))+
  geom_line()
```
There does not seem a consistent trend throughout the years. We see a dip after 1640 followed by a consistent upwards trajectory after 1660. There is a small dip at around 1665 but quickly recovered and the number of girls getting baptized was trending upwards after. 
...
ggplot(data=arbuthnot, aes(x= year, y=boy_ratio))+
  geom_line()
```
I see no clear trend. Theres constant fluctuation throughout the years signifying no real pattern and the average remaining at around a 0.52 ratio.
...
names(present)
```
The years ranges from 1940 to 2002. The data frame has dimensions 63x3. The column names are year, boys, girls.

1.  How do these counts compare to Arbuthnot's? Are they of a similar magnitude?
No, these counts in the present day are at a much higher magnitude. The population growth rate exploded over the past few centuries and the data here supports that. 
**Insert your answer here**
...
```{r}
present <- present %>%
  mutate(ratio = boys/(boys+girls))
ggplot(data=present, aes(x=year, y=ratio))+
  geom_line()

```
I see a overall downtrend throughout the years with slight recoveries in between. The observation about boys being born in greater proportions does hold up in the united states. You see a ratio of higher than 0.5 through the modern data.
...
present %>%
  ...
  arrange(desc(total))
```
We see most amount of births in the years 1961.
**Insert your answer here**
