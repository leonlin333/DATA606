...
This is an observational study. There is no treatment done by the researchers which means its not possible to isolate different variables and determine which are confounding variables. A better rephrased question would be whether there is an association between a professors physical appearance and their course eval score.
...
**Insert your answer here**
```{r}
ggplot(evals, aes(x = score)) + geom_histogram(binwidth = 0.1)
```
The distrution is left skewed. It is whats expected because most dissatisfied students don't tend to leave negative ratings.
3.  Excluding `score`, select two other variables and describe their relationship 
    with each other using an appropriate visualization.

**Insert your answer here**
```{r}
ggplot(evals, aes(x = age, y = bty_avg)) + geom_point() + geom_smooth(method = "lm", se = FALSE)
```
age and bty_avg shows a negative relationship. older professsors tend to recieve slightly lower beauty ratings on average. This is consistent with societal beauty norms but it is important to know that the relationship is not strong.
...
**Insert your answer here**
The initial scatter plot was becuase they share the same x and y values, causing points to overlap and make it look like a single point. Adding jitter makes it clearer and makes points on top of each other easily distinguishable. 
...
**Insert your answer here**
The equation is: yhat = 3.8803 + 0.0666 × bty_avg

The slope indicates that each one value increase in average beauty rating will casusethe predicted evaluation score increases by about 0.067 points. The pvalue for bty_avg is approximately 0.0001, making it statistically significant. 
...
```
The plots for m_bty_gen look similar to those from m_bty. Thr residual vs fitted doesn't signify a strong pattern and the QQ plot shows a roughly normal distribution with minor left skew. 

8.  Is `bty_avg` still a significant predictor of `score`? Has the addition
    of `gender` to the model changed the parameter estimate for `bty_avg`?

**Insert your answer here**
Yes the bty_avg is still a significant predictor of score. Adding gender slightly increases the estimate which means some sort of confounding effect.
...
yhat= (betahat_0 + betahat_color)+ betahat_bty_avg x btw_avg
...
summary(m_bty_rank)
```
R handles categorical variables with k levels by creating a k-1 dummy variable. 
...
**Insert your answer here**
cls_prof is the variable to have the highest expected p value. The amount of professors teaching the course shouldn't really affect the students rating on the course. 
...
**Insert your answer here**
cls_profs has the highest p value of 0.77 confirming our suspiciions from the previous exercise. 

13. Interpret the coefficient associated with the ethnicity variable.

**Insert your answer here**
The coefficient for ethnicity variable is around 0.123 which means that holding all other variables constant, professors who are not members of a minority group are expected to receive evaluation scores about 0.12 points higher than minority professors on average. 
...
summary(m_full2)
```
Dropping cls_prof causes minimal changes in the other coefficients and the other variables significant values. This indicates a low colinearlity. 
...
**Insert your answer here**
This information can have an impact for the independent conditions. Each row is a course and some professors can appear multiple times. Some courses can be non independent observations. 
...
**Insert your answer here**
Based on my final model a professor associated witha higher evaluation score would tend to be male, not a minority, and would have their degree from an english speaking school, and younger. 
...
No i would not be comfortable generalizing broadly. The data comes  exclusively from one institution with a specific student population and culture in that school. These factors that affects evaulation culture varys in every instituion and can all have their different systems. 
