...
ggplot(data = hfi, aes(x = pf_expression_control, y = pf_score)) +
  ...
  labs(
    x = "Political Pressure on Media (pf_expression_control)",
    y = "Personal Freedom Score (pf_score)",
    title = "Personal Freedom Score vs. Expression Control"
  )
```
A scatter plot dislats the relationship between two numerical variables. There is a positive linear trend so i would be confortable using a linear model to predict personal freedom score.
...
**Insert your answer here**
The previous graph shows a positive linear relationship. The relationship looks fairly strong with one variable increasing with the other variable. There are also no large outliers at both end of the variable so there is no observation large enough to fundamentally change the direction. 

Just as you've used the mean and standard deviation to summarize a single variable, you can summarize the relationship between these two variables by finding the line that best follows their association. Use the following interactive function to select the line that you think does the best job of going through the cloud of points.

```{r plotss-expression-score-1, eval=FALSE}
...
DATA606::plot_ss(x = hfi$pf_expression_control, y = hfi$pf_score)
```
After running it several times, the smallest sum of squares I got is around 950. 
## The linear model
...
summary(m2)
```
The slope of 0.349 tells us that every for evert one unit increase in political pressure/control on media content , the predicted total human freedom score increases subsequently by about 0.35 points. Countries with lower political interference in media tend to have meaningfully higher overall human freedom scores.
...
# Find an actual observation near 6.7 to compute residual
hfi %>%
  filter(round(pf_expression_control, 1) == 6.7) %>%
  select(countries, pf_expression_control, pf_score) %>%
  mutate(predicted = predicted, residual = pf_score - predicted)
```
Someone using a personal freedom score would predict a score of about 7.91. The overestimate or underestimate all depends on the country. A positive residual would mean the model underestimated while a negative residual would me the model overestimated. 
## Model diagnostics
...
**Insert your answer here**
There is no strong  pattern in the residuals v fitted values plot. The points are scattered sort of randomly around the dashed line at 0. This indicates that the linearity condition can be considered met. There is a slight suggestion of more spread at higher fitted values
...
**Insert your answer here**
The histogram of residuals is roughly bellshaped and centered near zero with a slight left skew. The QQ plot shows points following the diagonal line, with some deviation around the tails. Overall, the nearly normal residuals condition can be considered met despite it not being perfect
...
**Insert your answer here**
The residuals v fitted plot has the vertical spread of residuals that appears roughly consistent across the range of fitted values. The constant variability condition is reasonably satisfied for this model
* * *
...
**Insert your answer here**
```{r}
ggplot(data = hfi, aes(x = pf_rol, y = pf_score)) +
  geom_point() +
  stat_smooth(method = "lm", se = FALSE) +
  labs(
    x = "Rule of Law Score (pf_rol)",
    y = "Personal Freedom Score (pf_score)",
    title = "Personal Freedom Score vs. Rule of Law"
  )

m3 <- lm(pf_score ~ pf_rol, data = hfi)
summary(m3)
```
At a glance there doesn't appear to be a linear relationship
...
cat("R-squared (pf_expression_control -> pf_score):", summary(m1)$r.squared, "\n")
cat("R-squared (pf_rol -> pf_score):", summary(m3)$r.squared, "\n")
```
The r2 for pf expression control is around 0.634 while the other is around 0.57. This is slightly lower and means that the pf expression control is actually slightly better predictor of personal freedom score than the law alone. 
...
ggplot(data = hfi, aes(x = ef_score, y = pf_score)) +
  ...
  labs(
    ...
    title = "Personal Freedom vs. Economic Freedom"
  )
...
ggplot(data = m4, aes(x = .fitted, y = .resid)) +
  ...
  labs(x = "Fitted Values", y = "Residuals",
       title = "Residuals vs Fitted: ef_score -> pf_score")
...
The relationship that surprised me the most is the ef score and the pf score. I expected both of them to be largely independent with one relecting market structures and the other for civil liberties. Both of them have larger correlations than I thought. 
* * *
