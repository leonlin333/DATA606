...
A streak length of 1 mean that he hit one basket before missing. A streak length of 0 means means there was no hit and just miss after miss. There were 0 hits before a miss. 
...
kobe_streak_df <- data.frame(length = kobe_streak)
```

We can then take a look at the distribution of these streak lengths.

```{r plot-streak-kobe}
ggplot(data = kobe_streak_df, aes(x = length)) +
  geom_bar()
```
...
**Insert your answer here**
The distribution of the streak length is right skewed with the majority of the streak being 0 and the least amount of streak to be 4. His typical length streak is 0 followed by 1. His longest streak of baskets was 4.
## Compared to What?
...
18 coins came up heads while 82 came up tails
...
**Insert your answer here**
The change we need to make is edit the sample size from 1 to 133 and add the probabiltiy parameters to make it 0.45 for Hit and a 0.55 for a miss. 
...
sim_streak_df <- data.frame(length = sim_streak)
```
...
```{r}
ggplot(data = sim_streak_df, aes(x = length)) +
  geom_bar()
```
The typical length streak for this independent shooter with a 45% shooting is 0-1 with 0 being the most common. The players longest streak in 133 shots is 5.
...
**Insert your answer here**
If we ran the simulation a second time the streak distrubtion would look similar but not exactly the same. The shots are randomly generated so the exact data won't be replicated but it is likely that the distrubition or pattern would be intact. The frequency would be different but the graph would still be right skewed. 
...
**Insert your answer here**
Kobe Bryant’s distribution of streak lengths is very similar to the distribution of streak lengths for the simulated independent shooter. In both simulations the streak length of 0 was the highest followed by 1. Kobes does have longer streaks but the difference can be chalked up to random chance and a low sample size. There is no strong evidence that the hot hand model fits kobe shooting patterns because of the random fluctuations and the short streaks compared to the others.
