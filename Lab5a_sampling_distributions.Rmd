...
```{r}
  ...
  mutate(p = n /sum(n))
```
The distribution is different from before. There is now 0.7 benefits to the 0.3 doesn't benefit compared to the 0.8 benefit to the 0.2 doesnt benefit before. 
...
**Insert your answer here**
I would not expect the sample proportion to match the sample for another student because the sample is taken at random. The likelihood of having the same proportion with another student is very low however there is a high likelihood that the sample is similar if we take enough samples. The samples should be similar if the sample size is sufficiently large. Other students have around the same sample proportions but not exact same.
...
samp2 %>%
  ...
  mutate(p_hat = n /sum(n))
```
The proportion of samples from samp1 to samp2 is similar but still different. This is to be expected as samples are almost never the same and vary during each batch. If we took two more samples of size 100 and 1000 then you'll see a trend where the larger sample sizes give you the best proportions closest to the actual population. The higher the sample size the better the representation to the actual population. 
...
**Insert your answer here**
There are 15000 elements in sample_props50. The sampling distriution is looking closer to a normal distribution. The highest count is around 1.8-2 whih is the percentage from the data that says it doesn't benefit.
...
sample_props_small
```
There are 25 observations in this object. Each observation represents one sample of proportions of doesn't benefit from the actual population of size 10. 
...
The mean becomes closer and closer to the true mean of the population. The standard error decreases with increase in sample size. The distribution becomes more and more of a normal curve as the number of samples taken increase.
...
sample15 %>%
  ...
  mutate(p_hat = n / sum(n))
```
The best point estimate of a population proportion is the sample proportion
...
global_monitor %>%
  ...
  mutate(p = n /sum(n))
```
The shape of the data is sightly left skewed. The peak and highest count is 0.8.  I would guess the true proportion of those scientists would be around that number. The actual proportion is 0.8.
...
```
The distribution of this sample is starting to look more like a normal dsitribution. The previous one was all over the place but this one is starting to form some reminisce of a bell-shaped curve. Based on this sampling distrubution I would say the true proportion is around 0.8 again. 
...
Sample 3 has a smaller spread because it has more samples and has a larger sample size. Larger samples creates less variability and shows the true mean and is the better representation of the actual population. If you want the closest representation of the true population then a small spread is preffered. 
* * *
