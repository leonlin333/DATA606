...
```{r}
no_helmet %>%
  summarise(prop = mean(text_while_driving_30d == "30", na.rm = TRUE))
```
...
no_helmet %>%
  ...
  mutate(active_ind = ifelse(physically_active_7d == 0, "yes", "no")) %>%
...
no_helmet %>%
  ...
  mutate(sleep_ind = ifelse(school_night_hours_sleep %in% c("4", "5", "6", "7"), "yes", "no")) %>%
...
**Insert your answer here**
The relationship for p and me is an inverted U shape curve or concaved down. As the population proportion increases till 0.5 the margin of error decreases. After 0.5 proportion the margin of error decreases symmetrically. The margin of error is maximized at  p=0.5. 
...
**Insert your answer here**
The center is approximately 0.1, matching the true population proportion while the spread is narrow with a majority of the data being between 0.05 and 0.15. The shape looks to be pretty uniform but it is very slightly right skewed because p is closer to the left tail. 
...
**Insert your answer here**
the center shifts to match whatever p is set to. The sampling distribution is centered at the true p.
The shape becomes more symmetric and bell-shaped as p approaches 0.5. When p is near 0 or 1, the distribution becomes more skewed depending on whether its closer to 0 or 1.The spread is widest at p = 0.5 and becomes more narrows as p moves toward 0 or 1.   

8.  Now also change $n$. How does $n$ appear to affect the distribution of $\hat{p}$?

**Insert your answer here**
Spread decreases substantially. The distribution becomes much more tightly clustered around p. The shape becomes more normal as n increases, even when p is close to 0 or 1. The center stays the same and remains at p regardless of sample size.
...
yrbss %>%
  ...
  specify(response = strength_daily, explanatory = sleep_10plus, success = "yes") %>%
...
```
The pvalue is approximately around 0 which is less than the  significance level of 0.05. We would reject the null hypothesis.There is evidence that students who sleep 10+ hours per day are more likely to strength train every day of the week.
...
**Insert your answer here**
If there is no difference in likeliness to strenght train between two groups then the probability of detecting a difference at a 0.05 significance level is exactly 0.05 or 5%. Type 1 error is when you reject a true null hypothesis. 
...
**Insert your answer here**
Since there is no prior knowledge on p then the worst case scenario is that the margin of error would be maxed. You would need to sample at least 9604 people for a 95% confidence margin of error thats no greater than 1% 
