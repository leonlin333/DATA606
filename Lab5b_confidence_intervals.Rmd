...
samp %>%
  ...
  mutate(p = n /sum(n))
```
70% of the adults in our sample think climate change affects their community.

1.  Would you expect another student's sample proportion to be identical to yours? 
    Would you expect it to be similar? Why or why not?

**Insert your answer here**
No I would not expect another students proportions to be similar to mine because its a random sample with replacement. You're both picking from a pile and the chances of getting the same is very low.
...
**Insert your answer here**
95% confidence means to give us the upper and lower bound of our data where we're confident our data will be in that range 95% of the time. 
...
**Insert your answer here**
The confidence interval does capture the true population of the US who thinks climate change affects their community. The true population proportion is within the confidence interval.
...
**Insert your answer here**
You  would expect about 95% confidence interal to capture the true population mean. We used a 95% confidence interval where if we take multiple samples about 95% of those intervals would fall in between the confidence interval. 
...
**Insert your answer here**
About 95% of the confidence intervals would be expected to capture the true population's proportion. This proportion is not exactly equal to the confidence level in a single simulation becuase of sampling variabilty. By increasing the repetitions the proportion of intervals that capture the true value would slowly approach 95%.
* * *
...
**Insert your answer here**
I chose a 90% confidence interval. I would expect a 90% CI to be narrower because a lower CI means less certainty and the margin of error decreases. Less confidence of capturing the true population. Higher CI means wider while lower CI means lower. 
...
CI_90<- samp %>%
  ...
  get_ci(level = 0.90)
```
A 90% confidence interval means I'm 90% certain the the true population proportion falls between the upper and lower bound of the CI. The interval is slightly narrower because of lower CI. 
...
**Insert your answer here**
Using the app with a 90% confidence level and 50 intervals, I would expect about 90% of the intervals to contain the true population proportion. It is important to know there is sampling variability so the number isn't always exact. The proportion that captures the true value might be a little higher or lower each try, but over increase repetitions should get it closer to the selected confidence level.
...
1.  Lastly, try one more (different) confidence level. First, state how you expect the
    ...
    proportion.
**Insert your answer here**
Using the app with a 99% confidence interval I would expect 99% confidence that the true population is within the upper bound nad lower bound of the CI. After seeing the app and the infer package you see that theres less red lines. You would expect the interval to be much wider. 
```{r}
CI_90<- samp %>%
  specify(response = climate_change_affects, success = "Yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
...
**Insert your answer here**
As sample size increases the confidence interval becomes narrower. As it decreases the CI becomes wider. Larger samples have less variability and a smaller standard error which leads to a better estimate. Small sample sizes have high variability and can be outside of the CI. 
...
**Insert your answer here**
if the sample size is the same then increasing the bootstrap samples doesn't really change the width of the confidence interval. The standard error depends on the sample size so changing the bootstrap samples doesn't really impact it that much. Increasing the bootstrap just makes the estimates more stable but doesn't really change the standard error. 
* * *
